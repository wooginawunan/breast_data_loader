...
def random_augmentation_best_center(image, input_size, random_number_generator,
                                    ...
                                    max_rotation_noise=0, library='scipy', 
    ...
    if max_rotation_noise > 0:
        ...
        image = simple_rotation(image, angle, library=library)
        if auxiliary_image is not None:
            auxiliary_image = simple_rotation(
                auxiliary_image, angle, library=library
            )
    ...
    sampled_joint_image = _crop_image(
        padded_joint_image, input_size, borders, library
    )
...
def simple_rotation(image_to_rotate, angle, library='scipy'):
    '''
    ...
    if library == 'scipy':
        image_rotated = scipy.ndimage.interpolation.rotate(
            image_to_rotate, angle, order=3
        )
    elif library == 'cv2':
        ...
        image_rotated = skimage.transform.rotate(
            image_to_rotate, angle, order=3, resize=True
        )
    else:
...
def simple_resize(image_to_resize, size, library='scipy'):
    '''
    ...
    if library == 'scipy':
        image_resized = scipy.misc.imresize(
            image_to_resize, size, interp='bicubic'
        )
    elif library == 'cv2':
        ...
        if (len(image_to_resize.shape) == 3 and len(image_resized.shape) == 2 \
                and image_to_resize.shape[2] == 1):
            
            image_resized = np.expand_dims(image_resized, 2)

    elif library == 'skimage':
        image_resized = skimage.transform.resize(
            image_to_resize, size, order=3
        )
    else:
        raise KeyError(library)
...
@deprecated
def _calculate_cropping_border(image_size, input_size, random_number_generator,
                               max_crop_noise):
    '''
    deprecated function that calculates cropping border 
    from the left-middle of the image

    Returns:
    border (numpy array of 4 ints)
    '''
    ...
    if max_crop_noise.any():
        ...
        crop_noise_multiplier[0] = random_number_generator.uniform(
            low=-1.0, high=1.0
        )
        crop_noise_multiplier[1] = random_number_generator.uniform(
            low=0.0, high=1.0
        )
        # the breast is shifted to the left, there is no point in 
        # expanding the image on the left

    vertical_margin = int((image_size[0] - input_size[0]) / 2)

    top = vertical_margin
    bottom = vertical_margin + input_size[0]
    left = 0
    right = input_size[1]

    borders = np.array((top, bottom, left, right), dtype=np.int32)

    # if the maximum noise is too large and might put the crop outside of 
    # the image, it has to be made smaller for this image
    top_margin = top
    bottom_margin = image_size[0] - bottom
    right_margin = image_size[1] - right

    min_vertical_margin = min(top_margin, bottom_margin)
    min_horizontal_margin = right_margin

    if min_vertical_margin < max_crop_noise[0]:
        max_crop_noise[0] = min_vertical_margin

    if min_horizontal_margin < max_crop_noise[1]:
        max_crop_noise[1] = min_horizontal_margin
    ...
    assert ((borders[0] >= 0) and (borders[1] <= image_size[0]) \
                and (borders[2] >= 0) and (borders[3] <= image_size[1])), \
           "Centre of the crop area is sampled such that the borders" \
           " are outside of the image. Borders: " + str(borders) \
           + ', image shape: ' + str(image_size)
...
def _crop_image(image, input_size, borders, library):
    '''
    ...
    if (((borders[1] - borders[0]) != input_size[0]) 
            or ((borders[3] - borders[2]) != input_size[1])):
        cropped_image = simple_resize(
            cropped_image, input_size, library=library
        )
...
@deprecated
def random_augmentation(image, input_size, random_number_generator, 
                        max_crop_noise=(0, 0), max_crop_size_noise=0,
                        max_rotation_noise=0, library='scipy', 
                        auxiliary_image=None):
    '''
    Apply random noise in location and size of border and crop image within.
    ...
    borders = _calculate_cropping_border(
        image.shape, input_size, random_number_generator, max_crop_noise
    )
    borders = _resize_randomly_border(
        image, 
        input_size, 
    ...
    sampled_image = _crop_image(image, input_size, borders, library)
    ...
    if auxiliary_image is not None:
        ...
        sampled_auxiliary_image = _crop_image(
            auxiliary_image, input_size, borders, library
        )
    else:
        sampled_auxiliary_image = None

    return sampled_image, sampled_auxiliary_image
